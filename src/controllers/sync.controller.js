...
import { JWT_CONSTANTS } from "../constants/headers.constants.js";
import { REQUEST_TIMEOUTS } from "../constants/app.constants.js";

// ─── Helper: Generate signed JWT token for Monday callback
const getAuthToken = () => {
  const config = getConfig();
  const signingSecret = config.MONDAY_SIGNING_SECRET;
  const appId = Number(config.MONDAY_APP_ID);
...
export const syncLinkedInPosts = async (req, res) => {
  ...
  try {
    ...
    if (!shortLivedToken || !accountId) {
      ...
      logger.error(`[sync] ACTION: Verify Monday trigger is sending valid JWT`);
    ...
    logger.info(`[sync] Callback URL: ${callbackUrl ? "Present" : "Missing"}`);

    // ── All work runs synchronously, response sent only after completion ──────
    // Wait for the complete process to finish before sending response
    try {
      ...
      const boardId = String(
        payload?.inputFields?.boardId || config.MONDAY_CAMPAIGN_BOARD_ID || "",
      );
      ...
      let posts = [];
      try {
        posts = await fetchLastThreeMonthsPosts();
        logger.info(`[sync] Posts fetched: ${posts.length} posts from 90 days`);
      } catch (err) {
        logger.error(`[sync] LINKEDIN FETCH FAILED: ${err.message}`);
        logger.error(
          `[sync] Likely cause: LinkedIn token expired or invalid in .env`,
        );
        await sendCallback(callbackUrl, false);
        return res.status(StatusCodes.UNAUTHORIZED).json({
          success: false,
          ...
          details:
            "The LINKEDIN_ACCESS_TOKEN in .env may be expired. Please refresh the token.",
          message: err.message,
        });
      }
      ...
      if (posts.length === 0) {
        ...
        return res.status(StatusCodes.OK).json({
          success: true,
          message: "Sync completed - no posts to sync",
          summary: {
            total: 0,
            created: 0,
            updated: 0,
      ...
      for (const post of posts) {
        ...
        if (
          ...
          logger.error(`[sync] TOKEN INVALID - Storage service returned 401`);
          logger.error(`[sync] Details: ${postCheckResult.error}`);
          await sendCallback(callbackUrl, false);
          return res.status(StatusCodes.UNAUTHORIZED).json({
            success: false,
            error: "Authentication failed during sync",
            details: postCheckResult.error,
          });
        }
        ...
        if (!existingPost) {
          ...
          if (analyticsChanged) {
            ...
            if (updateRes.success) {
              ...
              logger.error(`[sync] Storage update failed: ${updateRes.error}`);
            }
            ...
            if (boardItemId) {
              try {
                ...
                logger.error(`[sync] Board update failed: ${err.message}`);
              }
            } else {
      ...
      // Return 200 OK with results after complete process is done
      return res.status(StatusCodes.OK).json({
        success: true,
      ...
      logger.error("[sync] SYNC PROCESS FAILED");
      logger.error(`[sync] Error: ${syncErr.message}`);
      logger.error(`[sync] Stack: ${syncErr.stack}`);
      logger.error(
        "═════════════════════════════════════════════════════════════",
      );
      await sendCallback(callbackUrl, false);
